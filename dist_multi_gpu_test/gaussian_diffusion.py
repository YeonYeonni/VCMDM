...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.lambda_angular_vel = lambda_angular_vel
    ...
    def compute_angular_velocity(self, motion_data, pose_rep='rot6d', mask=None, order='xyz'):
        ...
        # 4. Wrap to [-180, 180] (Gimbal Lock 처리)
        angular_velocity = (angular_velocity + 180) % 360 - 180 # [bs, nframes-1, 21, 3]
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        ...
        :param model: FlowMDM 모델. the model to evaluate loss on.
        :param x_start: [bs, 263, 1, 196] - Ground truth motion data.
        :param t: [bs] - Sampled timesteps (예: [342, 782, 91, ...])
        :param model_kwargs: if not None, a dict of extra keyword arguments to pass to the model. This can be used for conditioning. {'y': {'text', 'mask', 'angular_velocity', ...}}
        :param noise: if specified, the specific Gaussian noise to try to remove. Optional 사전 정의된 노이즈
        :return: a dict with the key "loss" containing a tensor of shape [N]. Some mean or variance settings may also have other keys.
        """

        # ===== 준비 작업 =====
        #enc = model.model # FlowMDM 모델
        
        enc = model.module.model if hasattr(model, 'module') else model.model # FlowMDM 모델 (DDP 대응)
        
        mask = model_kwargs['y']['mask'] # [bs, 1, 1, 196] - padding mask
        
        # xyz 변환 함수 (기하학적 loss 계산용)
        ...
        # ===== 1. Noise 생성 =====
        if noise is None:
            noise = th.randn_like(x_start) # [bs, 263, 1, 196] - Gaussian noise
            
        # ===== 2. Forward Diffusion (q_sample) =====
        # x_t = sqrt(alpha_bar_t) * x_0 + sqrt(1 - alpha_bar_t) * noise
        x_t = self.q_sample(x_start, t, noise=noise) # x_t: [bs, 263, 1, 196] - Noisy motion at timestep t # x_t = √α_t * x_0 + √(1-α_t) * noise

        terms = {}

        # ===== 3. 모델 예측 (FlowMDM Forward) =====
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            model_output = model(x_t, self._scale_timesteps(t), **model_kwargs) # model_output: [bs, 263, 1, 196] - 모델이 예측한 값
            ...
            # ===== 4. Target 결정 =====
            target = {
                ModelMeanType.PREVIOUS_X: self.q_posterior_mean_variance( # x_{t-1} 예측
                    x_start=x_start, x_t=x_t, t=t
                )[0],
                ModelMeanType.START_X: x_start, # x_0 직접 예측
                ModelMeanType.EPSILON: noise, # Noise 예측 (DDPM 방식) # 보통 EPSILON 사용 → target = noise
            }[self.model_mean_type]
            assert model_output.shape == target.shape == x_start.shape  # [bs, njoints, nfeats, nframes]

            # ===== 5. Rotation MSE Loss (Main Loss) =====
            terms["rot_mse"] = self.masked_l2(target, model_output, mask) # mean_flat(rot_mse)
            # L2 loss with masking (패딩 영역 제외)
            # [bs, 263, 1, 196] vs [bs, 263, 1, 196] → [bs] (각 샘플별 loss)

            target_xyz, model_output_xyz = None, None

            # ===== 6. Position-based Loss (lambda_rcxyz > 0) =====
            if self.lambda_rcxyz > 0.:
                target_xyz = get_xyz(target)  # [bs, nvertices(vertices)/njoints(smpl), 3, nframes] # [bs, 22, 3, 196] - GT joint positions
                model_output_xyz = get_xyz(model_output)  # [bs, nvertices, 3, nframes] # [bs, 22, 3, 196] - Predicted positions
                terms["rcxyz_mse"] = self.masked_l2(target_xyz, model_output_xyz, mask)  # mean_flat((target_xyz - model_output_xyz) ** 2)

            # ===== 7. Velocity Loss (lambda_vel > 0) =====
            ...
            # ===== 8. Foot Contact Loss (lambda_fc > 0) =====
            if self.lambda_fc > 0.:
                ...
                if self.data_rep == 'rot6d' and dataset.dataname in ['humanact12', 'uestc']:
                    ...
                    # Foot가 지면에 닿았을 때 velocity가 0이어야 함
                    gt_joint_vel = torch.linalg.norm(gt_joint_xyz[:, :, :, 1:] - gt_joint_xyz[:, :, :, :-1], axis=2)  # [BatchSize, 4, Frames]
                    fc_mask = torch.unsqueeze((gt_joint_vel <= 0.01), dim=2).repeat(1, 1, 3, 1) # Velocity 임계값 이하 = foot contact
                    pred_joint_xyz = model_output_xyz[:, relevant_joints, :, :]  # [BatchSize, 4, 3, Frames]
                    pred_vel = pred_joint_xyz[:, :, :, 1:] - pred_joint_xyz[:, :, :, :-1]
                    pred_vel[~fc_mask] = 0 # Non-contact 영역은 무시
                    terms["fc"] = self.masked_l2(pred_vel, # Contact 시 velocity 0 강제
                                                 torch.zeros(pred_vel.shape, device=pred_vel.device),
                                                 mask[:, :, :, 1:])
            
            # ===== 9. Velocity Loss (lambda_vel > 0) =====
            if self.lambda_vel > 0.:
                target_vel = (target[..., 1:] - target[..., :-1]) # [bs, 263, 1, 195] - Frame difference
                model_output_vel = (model_output[..., 1:] - model_output[..., :-1])
                terms["vel_mse"] = self.masked_l2(target_vel[:, :-1, :, :], # Remove last joint, is the root location! # Remove root joint
                                                  model_output_vel[:, :-1, :, :],
                                                  mask[:, :, :, 1:])  # mean_flat((target_vel - model_output_vel) ** 2)


            # ===== 10. Angular Velocity Loss (lambda_angular_vel > 0) =====
            # if self.lambda_angular_vel > 0.:
            #     # target의 angular velocity 계산
            #     target_angular_vel = self.compute_angular_velocity(target, enc.pose_rep, mask)  # [bs, 21, 3, nframes-1]
                
            #     # model_output의 angular velocity 계산
            #     model_output_angular_vel = self.compute_angular_velocity(model_output, enc.pose_rep, mask)  # [bs, 21, 3, nframes-1]
                
            #     # Loss 계산 (패딩 영역 제외)
            #     terms["angular_vel_mse"] = self.masked_l2(
            #         target_angular_vel,
            #         model_output_angular_vel,
            #         mask[:, :, :, :-1]  # [bs, 1, 1, nframes-1]
            #     )
                
            #     print(f"\n[GAUSSIAN_DIFFUSION DEBUG] Angular Velocity Loss computed!", flush=True)
            #     print(f"[GAUSSIAN_DIFFUSION DEBUG] target_angular_vel shape: {target_angular_vel.shape}", flush=True)
            #     print(f"[GAUSSIAN_DIFFUSION DEBUG] angular_vel_mse: {terms['angular_vel_mse'].mean().item():.6f}", flush=True)
            #     print(f"[GAUSSIAN_DIFFUSION DEBUG] Adding to terms dict\n", flush=True)

            # ===== 10. Angular Velocity Loss (lambda_angular_vel > 0) =====
            if self.lambda_angular_vel > 0.:
                # GT angular velocity from pre-computed dataset file
                gt_angular_vel = model_kwargs['y']['angular_velocity']  # [bs, 196, 63]
                
                # Reshape: [bs, 196, 63] → [bs, 21, 3, 196]
                bs_av, nframes_av, _ = gt_angular_vel.shape
                gt_angular_vel = gt_angular_vel.reshape(bs_av, nframes_av, 21, 3)  # [bs, 196, 21, 3]
                gt_angular_vel = gt_angular_vel.permute(0, 2, 3, 1)  # [bs, 21, 3, 196]
                
                # Adjust to T-1 frames (angular velocity는 frame difference이므로)
                actual_nframes = x_start.shape[-1]  # 196
                if gt_angular_vel.shape[-1] == actual_nframes - 1:
                    gt_angular_vel_trimmed = gt_angular_vel  # Already T-1
                elif gt_angular_vel.shape[-1] == actual_nframes:
                    gt_angular_vel_trimmed = gt_angular_vel[:, :, :, :-1]  # T → T-1
                else:
                    raise ValueError(f"Unexpected angular_velocity shape: {gt_angular_vel.shape}")
                
                # Compute predicted angular velocity from model output
                model_output_angular_vel = self.compute_angular_velocity(
                    model_output,  # [bs, 263, 1, 196]
                    enc.pose_rep,  # 'rot6d'
                    mask  # [bs, 1, 1, 196]
                )  # [bs, 21, 3, 195]
                
                # MSE Loss between GT and predicted angular velocity
                terms["angular_vel_mse"] = self.masked_l2(
                    gt_angular_vel_trimmed,  # [bs, 21, 3, 195] - Ground truth from file
                    model_output_angular_vel,  # [bs, 21, 3, 195] - Model prediction
                    mask[:, :, :, :-1]  # [bs, 1, 1, 195] - Mask (exclude last frame)
                )
            ...
            # ===== Total Loss =====
            '''
                Main rotation loss : terms["rot_mse"]
                Variational bound : terms["vb"] (if learned variance)
                Velocity loss : terms["vel_mse"]
                Position loss : terms["rcxyz_mse"]
                Foot contact loss : terms["fc"]
                Angular velocity loss : terms["angular_vel_mse"]
            '''
            terms["loss"] = terms["rot_mse"] + terms.get('vb', 0.) +\
                            (self.lambda_vel * terms.get('vel_mse', 0.)) +\
                            (self.lambda_rcxyz * terms.get('rcxyz_mse', 0.)) + \
                            (self.lambda_fc * terms.get('fc', 0.))
                            #(self.lambda_angular_vel * terms.get('angular_vel_mse', 0.))

        else:
            raise NotImplementedError(self.loss_type)

        return terms # {'loss': [bs], 'rot_mse': [bs], 'vel_mse': [bs], ...}
