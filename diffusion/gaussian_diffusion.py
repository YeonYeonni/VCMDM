...
from data_loaders.humanml.common.quaternion import cont6d_to_matrix, qeuler, matrix_to_quat, cont6d_to_matrix_np
...
class GaussianDiffusion:
    ...
    def compute_angular_velocity(self, motion_data, pose_rep='rot6d', mask=None, order='yxz'):
        bs, _, _, nframes = motion_data.shape # motion_data: [bs, 263, 1, nframes]
        eulers = []

        for i in range(bs):
            # 1. i번째 샘플의 rotation 데이터만 추출
            single_rot_data = motion_data[i, 67:193, 0, :]  # [126, nframes]
            single_rot_data = single_rot_data.permute(1, 0)  # [nframes, 126]
            single_rot_data = single_rot_data.reshape(nframes, 21, 6)  # [nframes, 21, 6]

            # 3. 6D to Rotation Matrix
            rot_matrix = cont6d_to_matrix(single_rot_data)  # [nframes, 21, 3, 3]

            # 4. Rotation Matrix to Quaternion
            quat = matrix_to_quat(rot_matrix)  # [nframes, 21, 4]

            # 5. Quaternion to Euler angles
            euler = qeuler(quat, order=order, deg=True)  # [nframes, 21, 3]
            eulers.append(euler)

        eulers = torch.stack(eulers, dim=0)  # [bs, nframes, 21, 3]
 
        # 6. 각속도 계산 (PyTorch로 계산)
        angular_velocity = eulers[:, 1:, :, :] - eulers[:, :-1, :, :]  # [bs, nframes-1, 21, 3]
 
        # 7. Shape 변환 (PyTorch permute 사용)
        angular_velocity = angular_velocity.permute(0, 2, 3, 1)  # [bs, 21, 3, nframes-1]
        euler_angles = eulers.permute(0, 2, 3, 1)  # [bs, 21, 3, nframes]

        return angular_velocity, euler_angles  # PyTorch tensor 반환 (각속도, 오일러 각도)
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            # if t[0] % 100 == 0:  # 100 스텝마다 출력 (너무 많은 출력 방지)
            #     print(f"\n[LOSS BREAKDOWN] Timestep: {t[0].item()}")
            #     print(f"  rot_mse:     {terms['rot_mse'].mean().item():.6f}")
            #     if 'vb' in terms:
            #         print(f"  vb:          {terms['vb'].mean().item():.6f}")
            #     if 'vel_mse' in terms:
            #         print(f"  vel_mse:     {terms['vel_mse'].mean().item():.6f} (lambda={self.lambda_vel})")
            #     if 'rcxyz_mse' in terms:
            #         print(f"  rcxyz_mse:   {terms['rcxyz_mse'].mean().item():.6f} (lambda={self.lambda_rcxyz})")
            #     if 'fc' in terms:
            #         print(f"  fc:          {terms['fc'].mean().item():.6f} (lambda={self.lambda_fc})")
            #     if 'angular_vel_mse' in terms:
            #         print(f"  angular_vel: {terms['angular_vel_mse'].mean().item():.6f} (lambda={self.lambda_angular_vel})")
            #     print(f"  TOTAL LOSS:  {terms['loss'].mean().item():.6f}\n", flush=True)
