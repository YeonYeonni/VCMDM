...
class FlowMDM(TextConditionalModel):
    def __init__(self, njoints, nfeats, translation, pose_rep, glob, glob_rot,
        ...
        ## Velocity Embedding 추가 실험 ##

        # w1 = max(self.latent_dim // 4, 32)
        # w2 = max(self.latent_dim // 2, 64)
        
        # self.angularVelEncoder = nn.Sequential(
        #     # 8 -> w1
        #     nn.Linear(8, w1),
        #     nn.SiLU(),
        #     nn.LayerNorm(w1),
        ...
        #     # w2 -> latent
        #     nn.Linear(w2, self.latent_dim),
        #     nn.SiLU(),
        #     nn.LayerNorm(self.latent_dim),
        # )
        
        # self.angularVelRes = nn.Sequential(
        #     nn.LayerNorm(self.latent_dim),
        ...
        # self.angularVelOutNorm = nn.LayerNorm(self.latent_dim)
        
        
        self.angular_vel_dim = 8

        self.angularVelTemporalEncoder = nn.Sequential(
            nn.Conv1d(self.angular_vel_dim, latent_dim // 2, kernel_size=3, padding=1),
            nn.SiLU(),
            nn.Conv1d(latent_dim // 2, latent_dim, kernel_size=3, padding=1),
            nn.SiLU(),
        )
        self.angularVelFrameProj = nn.Linear(self.angular_vel_dim, latent_dim)
        self.angularVelOutNorm = nn.LayerNorm(latent_dim)

        ## 여기까지 Velocity Embedding 추가 실험 ##
    ...
    def forward(self, x, timesteps, y):
        ...
        """
        bs, njoints, nfeats, nframes = x.shape # [bs, 263, 1, nframes]
        ...
        #print('x shape before input_process:', x.shape)
