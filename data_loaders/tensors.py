...
def collate(batch):
    ...
    # Angular velocity 추가
    if 'angular_velocity' in notnone_batches[0]:
        angular_velocities = [b['angular_velocity'] for b in notnone_batches]  # list of [196, 63]
        angular_velocities = collate_tensors(angular_velocities)  # [bs, 196, 63]
        cond['y'].update({'angular_velocity': angular_velocities.to(databatchTensor.device)})

    return motion, cond

# an adapter to our collate func # batch는 __getitem__의 반환값들의 리스트. 배치로 합쳐서 반환
def t2m_collate(batch):
    ...
    # Angular velocity 추가 (index 9)
    if len(batch[0]) >= 10:
        for i, b in enumerate(adapted_batch):
            b['angular_velocity'] = torch.tensor(batch[i][9]).float()  # [196, 63]
